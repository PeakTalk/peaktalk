    ...
    return (
        <div className="pb-10 pt-4 sm:pt-8 w-full max-w-6xl mx-auto px-6 lg:px-10">
            ...
            <div className="flex flex-col gap-6 mb-10 sm:flex-row sm:items-center sm:justify-between sm:mb-14">
                <div>
                    <div className="font-mono text-[11px] text-[var(--accent-primary)] tracking-[0.1em] uppercase mb-3">
                        БАЗА ЗНАНИЙ
                    </div>
                    <h1 className="font-syne text-3xl sm:text-4xl md:text-5xl font-bold text-[var(--text-main)] leading-tight tracking-tight m-0">
                        Мои документы
                    </h1>
                </div>
                <Link
                    href="/upload"
                    className="btn-primary w-full sm:w-auto mt-2 sm:mt-0 shadow-[0_0_20px_rgba(245,158,11,0.25)] hover:shadow-[0_0_30px_rgba(245,158,11,0.4)] transition-shadow"
                >
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-8">
                <div className="relative flex-1">
                    <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-dim)] pointer-events-none" />
                    <input
                        type="text"
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        placeholder="Поиск по названию..."
                        className="w-full bg-[var(--bg-card)] border border-[var(--border-main)] focus:border-[var(--accent-primary)] focus:ring-1 focus:ring-[var(--accent-primary)] rounded-xl py-3 pl-11 pr-10 text-sm text-[var(--text-main)] placeholder-slate-500 outline-none transition-colors font-inter"
                    />
                    {search && (
                        <button
                            onClick={() => setSearch('')}
                        ...
                        <button
                            ...
                            className={`px-4 py-3 rounded-xl text-sm font-mono border transition-colors whitespace-nowrap ${
                                activeFilter === f
                                    ...
                                    : 'bg-[var(--bg-card)] border-[var(--border-main)] text-[var(--text-muted)] hover:border-[var(--border-light)]'
                            }`}
                        >
                            {f}
                        </button>
            ...
            {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20">
                    <Loader2 size={28} className="animate-spin text-[var(--accent-primary)] mb-4" />
                    <div className="text-[var(--text-muted)] font-mono text-sm">Загрузка...</div>
                </div>
            ) : error ? (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <div className="text-[var(--text-main)] mb-2">Не удалось загрузить документы</div>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-[var(--border-main)] rounded-2xl bg-[var(--bg-surface)]">
                    <div className="text-[var(--text-dim)] mb-4 bg-[var(--bg-card)] p-4 rounded-full">
                        <UploadCloud size={28} />
                    </div>
                    <div className="text-[var(--text-main)] mb-2 font-syne text-xl">Нет загруженных документов</div>
                    <div className="text-[var(--text-dim)] text-sm mb-6 max-w-sm">
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    <p className="text-[var(--text-muted)] font-mono text-sm">Ничего не найдено по запросу «{search}»</p>
                    <button onClick={() => setSearch('')} className="mt-3 text-xs text-[var(--accent-primary)] hover:underline">
                        Сбросить поиск
                    </button>
                </div>
            ) : (
                        ...
                        const CardContent = (
                            <div
                                className={`bg-[var(--bg-card)] border rounded-2xl p-5 flex flex-col h-full relative overflow-hidden group transition-colors duration-200 ${
                                    ...
                                    analysisDraftId
                                        ? 'border-[var(--border-main)] hover:border-[var(--accent-primary)]/50 cursor-pointer'
                                        : 'border-[var(--border-main)] hover:border-[var(--border-light)]'
                                }`}
                            >
                                <div className="absolute -right-10 -top-10 w-32 h-32 bg-[var(--accent-primary)]/5 rounded-full blur-2xl group-hover:bg-[var(--accent-primary)]/10 transition-colors duration-500 pointer-events-none" />

                                <div className="flex justify-between items-start mb-4 relative z-10">
                                    <div className="w-11 h-11 rounded-xl bg-[var(--bg-surface-alt)] border border-[var(--border-light)] flex items-center justify-center text-[var(--text-muted)] group-hover:text-[var(--accent-primary)] transition-colors duration-200">
                                    ...
                                    <button
                                        ...
                                        disabled={isDeleting}
                                        className="p-2 text-[var(--text-dim)] hover:text-red-400 transition-colors rounded-lg hover:bg-red-400/10 cursor-pointer"
                                    >
                                ...
                                <div className="mb-4 relative z-10 flex-1">
                                    <h3 className="font-syne text-base font-semibold text-[var(--text-main)] mb-1 line-clamp-2 group-hover:text-[var(--accent-primary)] transition-colors duration-200" title={doc.name}>
                                        {doc.name}
                                    </h3>
                                    <span className="label-kicker bg-[var(--bg-surface-alt)] px-2 py-0.5 rounded border border-[var(--border-main)]">
                                        {ext}
                                    </span>
                                </div>

                                <div className="flex items-center justify-between mt-auto pt-4 border-t border-[var(--border-light)]/50 relative z-10">
                                    <div className="flex items-center gap-1.5 text-[var(--text-dim)] font-mono text-xs">
                                        <Calendar size={11} />
