...
import { FileText, UploadCloud, File, Search, Trash2, Loader2, RefreshCcw, ArrowRight, X, AlignLeft } from 'lucide-react';
...
function getExt(doc: Doc): string {
    ...
    if (['PDF', 'DOCX', 'DOC', 'TXT'].includes(ext)) return ext === 'DOC' ? 'DOCX' : ext;
    return 'ФАЙЛ';
}

function DocIcon({ doc }: { doc: Doc }) {
    if (doc.source === 'text') return <AlignLeft size={15} className="text-gray-400" />;
    const ext = doc.name.split('.').pop()?.toLowerCase() ?? '';
    if (ext === 'pdf') return <FileText size={15} className="text-gray-400" />;
...
export default function DocumentsPage() {
    ...
    return (
        <div className="pt-8 pb-16 w-full max-w-4xl mx-auto px-6">

            {/* Header */}
            <div className="flex items-center justify-between mb-8">
                <h1 className="text-2xl font-semibold text-gray-900">Мои тексты</h1>
                <Link
                    href="/upload"
            ...
            <div className="flex items-center gap-4 mb-6">
                ...
                {data && (
                    <span className="ml-auto text-xs text-gray-400">{filtered.length} из {data.total}</span>
                )}
            </div>
            ...
            {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 border-2 border-dashed border-gray-200 rounded-2xl">
                    <UploadCloud size={32} className="text-gray-300 mb-4" />
                    <p className="text-sm font-medium text-gray-700 mb-1">Пока нет материалов</p>
                    <p className="text-xs text-gray-400 mb-6">Загрузите файл или создайте текстовый документ</p>
                ...
                <div className="flex flex-col items-center justify-center py-16 text-center">
                    <p className="text-sm text-gray-400">Ничего не найдено по «{search}»</p>
                    <button onClick={() => setSearch('')} className="mt-2 text-xs text-orange-500 hover:underline">Сбросить</button>
                </div>
            ) : (
                <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
                    {/* Table head */}
                    <div className="grid grid-cols-[1fr_80px_100px_100px] bg-gray-50/70 border-b border-gray-100 px-5 py-2.5">
                    ...
                    {filtered.map((doc) => {
                        ...
                        return (
                            <div
                                key={doc.id}
                                className={`grid grid-cols-[1fr_80px_100px_100px] items-center px-5 py-3.5 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors group ${isDeleting ? 'opacity-40' : ''}`}
                            >
                                {/* Name */}
