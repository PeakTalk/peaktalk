...
function buildSummary(
    ...
    const intro =
        avgScore >= 8
            ? `Позиция выдержала давление ${personaName}. Структура и ключевые аргументы выглядят готовыми к реальной встрече.`
            : avgScore >= 6
            ? `Рабочая основа есть, но ${personaName} нашел зоны, где защиту нужно усилить фактами и формулировками.`
            : avgScore >= 4
            ? `Часть позиции выдержала, но ${personaName} выявил слепые зоны, которые опасно не закрыть до встречи.`
            : `${personaName} быстро нашел критичные разрывы в позиции. Перед встречей нужно пересобрать аргументы и ответы на ожидаемые возражения.`;
