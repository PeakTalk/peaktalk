...
def test_create_gemini_client_with_dedicated_proxy(monkeypatch) -> None:
    ...
    assert captured["api_key"] == "test-key"
    assert captured["http_options"].client_args == {"proxy": proxy_url, "trust_env": False}
    assert captured["http_options"].async_client_args == {"proxy": proxy_url, "trust_env": False}