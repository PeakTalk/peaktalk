...
_AI_DETECTION_SYSTEM = """You are an AI-content detector. Your task: decide if the given text was written by a human or generated by an AI assistant.

SCORE these signals (each adds evidence for AI authorship):
1. STRUCTURE — unnaturally perfect: numbered lists 1-2-3, bullet points, headers in a conversational reply
2. STYLE — corporate/AI-speak: "certainly", "of course", "it is important to note", "in conclusion", "firstly/secondly/thirdly", "key aspects"
3. GRAMMAR/PUNCTUATION — flawless formal Russian with zero colloquialisms, typos, or natural speech patterns
4. CONTENT — generic, hollow, no personal experience or concrete specifics; answers sound like a Wikipedia summary
5. LENGTH/BALANCE — suspiciously well-balanced paragraphs of equal length covering all angles evenly
6. RUSSIAN AI TELLS — phrases like "безусловно", "в заключение", "следует отметить", "ключевые аспекты", "важно понимать", "данный вопрос"

VERDICT RULES:
- 4+ signals present → ai_generated: true
- Text that sounds like a live spoken answer with natural pauses, mild grammar issues, or personal anecdotes → ai_generated: false
- Short conversational replies under 3 sentences → ai_generated: false (too short to judge)
- When uncertain → ai_generated: false (fail open)

Return ONLY valid JSON with no extra text."""

_AI_DETECTION_PROMPT = """Analyze this text for AI authorship signals:

---
{text}
---

JSON response: {{"ai_generated": true}} or {{"ai_generated": false}}"""


async def detect_ai_content(text: str) -> bool:
    """Returns True when text shows strong AI-generation signals (4+ markers).
    Fails open: returns False on any error to never block legitimate users."""
    if len(text) < 60:
        return False

    client = genai.Client(api_key=settings.gemini_api_key)
    prompt = _AI_DETECTION_PROMPT.format(text=text[:3000])
    ...
    try:
        ...
        return False  # Fail open — never block if detection fails
