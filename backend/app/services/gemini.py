...
_AI_DETECTION_SYSTEM = """You are an expert AI-content detector. Your task is to analyze the provided text (which is supposedly a spoken answer) and determine if it was written by a human or generated by an AI (like ChatGPT or Claude).

Focus on these AI signals:
1. STRUCTURE & FORMATTING: Unnatural use of markdown, bullet points, numbered lists, or bolding.
2. STYLE & VOCABULARY: Formal "AI-speak" patterns (e.g., "безусловно", "следует отметить", "важно подчеркнуть", "в заключение", "certainly", "in conclusion").
3. TONE & CONTENT: Generic, hollow statements lacking personal anecdotes, concrete numbers, or real-world friction. Preachy or overly balanced tone.
4. NATURALNESS: Real human answers have imperfections, colloquialisms, hesitations, or varied sentence lengths. AI text is often eerily uniform.

Return ONLY a JSON object: {"ai_generated": true} or {"ai_generated": false}"""


async def detect_ai_content(text: str) -> bool:
    """
    Smart AI detection using gemini-2.5-flash-lite. 
    It is extremely cheap and fast, so we always consult it for better accuracy.
    Falls back to heuristics if the API fails.
    """
    if len(text.strip()) < 60:
        return False  # Too short to reliably detect

    client = genai.Client(api_key=settings.gemini_api_key)
    prompt = f"Is this answer AI-generated?\n---\n{text[:2000]}\n---\nReturn JSON: {{\"ai_generated\": true}} or {{\"ai_generated\": false}}"
    ...
    try:
        ...
        print(f"AI detection API failed: {e}")
        # Fail open with a more sensitive heuristic fallback
        score, _ = _heuristic_score(text)
        return score >= 0.4
