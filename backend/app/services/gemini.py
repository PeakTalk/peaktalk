...
_AI_DETECTION_PROMPT = """Determine if the following text was ENTIRELY generated by an AI assistant (ChatGPT, Claude, Gemini, etc.).
Flag as AI-generated ONLY when you are HIGHLY confident — clear patterns include: perfect numbered/bulleted structure,
mechanical corporate-speak, overly balanced and symmetric paragraphs, generic AI-template phrases.
Well-written human text and minor AI assistance should NOT be flagged.
...
Respond ONLY with valid JSON: {{"ai_generated": true}} or {{"ai_generated": false}}"""


async def detect_ai_content(text: str) -> bool:
    """Returns True only when text is clearly and entirely AI-generated (high confidence).
    Fails open: returns False on any error to never block legitimate users."""
    if len(text) < 80:
        return False

    client = genai.Client(api_key=settings.gemini_api_key)
    prompt = _AI_DETECTION_PROMPT.format(text=text[:2000])
