...
async def get_current_user(
    ...
    if provider == "logto":
        return await _get_logto_user(token, db, credentials_exception)

    if provider == "dual-validate":
        try:
            unverified = jwt.decode(token, options={"verify_signature": False})
        except Exception:
            unverified = {}
        if unverified.get("iss") == settings.logto_issuer:
            return await _get_logto_user(token, db, credentials_exception)

    return await _get_legacy_user(
